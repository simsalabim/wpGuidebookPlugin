...
require_once 'utils/ArrayFunc.php';
...
class Active_Record {
  ...
  public function find($conditions = array()) {
    ...
    foreach ($conditions['fields'] as $key => $field) {
       ...
       if ($field != ArrayFunc::last($conditions['fields'])) {
         $where .= ' AND ';
      }      
    }
  ...
  protected function parseConditions($conditions) {
    $orderBy = ArrayFunc::get($conditions, 'orderBy', '');
    $orderBy = $orderBy ? ('ORDER BY ' . $orderBy) : '';
    unset($conditions['orderBy']);

    $limit = ArrayFunc::get($conditions, 'orderBy', '');
